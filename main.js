// main.js
import { initAuth, logout, sendMagicLink } from './auth.js';
import { loadData, saveData, siteData, addPhoto, deletePhoto, updatePhoto, uploadGenericImage, globalError, resetAllData } from './data.js';
...
// Theme Definitions
const THEMES = {
    'default': { '--bg-light': '#FFF8F0', '--bg-soft': '#FFEAE3', '--primary': '#E8A0BF', '--accent': '#BA90C6', '--text-dark': '#432C39', '--text-light': '#6D5D6E' }, // Romantic Rose
    'ocean': { '--bg-light': '#F0F9FF', '--bg-soft': '#E0F2FE', '--primary': '#38BDF8', '--accent': '#0284C7', '--text-dark': '#0C4A6E', '--text-light': '#475569' }, // Ocean Breeze
    'nature': { '--bg-light': '#F7FEE7', '--bg-soft': '#ECFCCB', '--primary': '#84CC16', '--accent': '#4D7C0F', '--text-dark': '#14532D', '--text-light': '#3F6212' }, // Sage & Forest
    'lavender': { '--bg-light': '#FAF5FF', '--bg-soft': '#F3E8FF', '--primary': '#A855F7', '--accent': '#7E22CE', '--text-dark': '#3B0764', '--text-light': '#6B21A8' }, // Lavender Haze
    'cherry': { '--bg-light': '#FFF1F2', '--bg-soft': '#FFE4E6', '--primary': '#FB7185', '--accent': '#E11D48', '--text-dark': '#881337', '--text-light': '#9F1239' }, // Cherry Blossom
    'sunshine': { '--bg-light': '#FEFCE8', '--bg-soft': '#FEF9C3', '--primary': '#FACC15', '--accent': '#CA8A04', '--text-dark': '#422006', '--text-light': '#713F12' }, // Golden Hour
    'coral': { '--bg-light': '#FFF7ED', '--bg-soft': '#FFEDD5', '--primary': '#FB923C', '--accent': '#EA580C', '--text-dark': '#431407', '--text-light': '#7C2D12' }, // Warm Coral
    'teal': { '--bg-light': '#F0FDFA', '--bg-soft': '#CCFBF1', '--primary': '#2DD4BF', '--accent': '#0D9488', '--text-dark': '#134E4A', '--text-light': '#115E59' }, // Serene Teal
    'mocha': { '--bg-light': '#F5F5F4', '--bg-soft': '#E7E5E4', '--primary': '#A8A29E', '--accent': '#57534E', '--text-dark': '#292524', '--text-light': '#44403C' }, // Soft Mocha
    'berry': { '--bg-light': '#FDF2F8', '--bg-soft': '#FCE7F3', '--primary': '#F472B6', '--accent': '#DB2777', '--text-dark': '#500724', '--text-light': '#831843' }  // Sweet Berry
};
...
async function init() {
    ...
    window.addEventListener('offline', () => showCustomAlert("Connection Lost", "You are offline.", true));
    ...
    const isReturningFromEmail = window.location.hash && window.location.hash.includes('access_token');
    if (isReturningFromEmail) loginOverlay.classList.add('hidden');
    ...
    if (!isReturningFromEmail) {
        const { data } = await import('./auth.js').then(m => m.getSession()); 
        if (!data.session) {
            loginOverlay.classList.remove('hidden');
            bindLoginEvents();
            document.body.classList.add('loaded');
...
function renderErrorState() {
    mainContent.innerHTML = `
        <div class="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
            <div class="text-red-400 text-6xl mb-4"><i class="fas fa-heart-broken"></i></div>
            <h2 class="text-2xl font-bold text-slate-700 mb-2">Unable to Load Our Story</h2>
            <p class="text-gray-500 mb-6">We couldn't connect to the database. Check your internet connection.</p>
            <button onclick="window.app.retryLoad()" class="btn btn-primary shadow-lg px-8 py-3">
                <i class="fas fa-redo mr-2"></i> Try Again
            </button>
        </div>
    `;
    if(sidebarName) sidebarName.textContent = "My Love";
}
...
function buildNavigation() {
    ...
    navLinks.forEach(link => {
        ...
        a.onclick = (e) => { e.preventDefault(); window.location.hash = link.id; };
        ...
        ma.onclick = (e) => { e.preventDefault(); window.location.hash = link.id; };
...
function navigateTo(pageId) {
    ...
    if (activeSection) {
        ...
        if (pageId === 'journey') {
            ...
            const journeyContent = `<div id="calendar-container" class="mb-8">${renderCalendar(currentCalendarDate)}</div><div class="border-t pt-6"><h3 class="text-xl font-bold mb-4">Our Timeline</h3><div id="timeline-list">${renderTimelineList()}</div></div>`;
...
async function saveEntry(pageId, index, context) {
    if (globalError) { showCustomAlert("Error", "Offline/Error state."); return; }
    ...
    try {
        ...
        if (pageId === 'gallery') {
             if (fileInput && fileInput.files.length > 0) {
                 if(isNew) await addPhoto(fileInput.files[0], val('caption'));
             } 
             if (!isNew) await updatePhoto(index, val('caption'));
        } 
        else {
            if (fileInput && fileInput.files.length > 0) uploadedUrl = await uploadGenericImage(fileInput.files[0]);
            const getImgVal = (id) => uploadedUrl || val(id); 

            if (pageId === 'home') { if (context === 'timer') siteData.homepage.relationshipStart = val('date'); else { siteData.homepage.mainImage = getImgVal('image'); siteData.homepage.introMessage = val('intro'); siteData.homepage.relationshipTag = val('tag'); } } 
            ...
            else if (pageId === 'surprise') { siteData.surprise = { title: val('title'), message: val('msg'), image: getImgVal('image') }; } 
            else if (pageId === 'wheel') {
            siteData.surprise.wheelItems = val('wheel-items').split(',').map(s => s.trim()).filter(s => s);
            pageId = 'surprise'; // navigate back to surprise after save
            }
        ...
        const sectionMap = { 'loveLanguages': 'allMyLove', 'loveReasons': 'allMyLove', 'notes': 'memories' };
...
function bindLoginEvents() {
    ...
    newBtn.addEventListener('click', async () => {
        ...
        if(!email) return;
        newBtn.disabled = true; newBtn.innerText = 'Sending...';
        const { error } = await sendMagicLink(email);
        if (error) { msg.textContent = "Error: " + error.message; newBtn.disabled = false; } 
        else { msg.innerHTML = "Magic link sent!<br>Check email."; msg.classList.add('text-green-600'); }
    });
}
